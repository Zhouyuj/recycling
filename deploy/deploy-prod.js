...
var remotes = {
  servers: [
    {
      host: '125.94.39.114',
      user: 'root',
      password: 'Locision@2017#production',
    },
    {
      host: '125.94.38.248',
      user: 'root',
  ...
  projectName: 'web-client-prod',
  projectSpace: '/root/deploy/community/community-web-client',
  scriptPath: '/root/deploy/community/community-web-client/web-middleware/web-client',
  backupPath: '/root/deploy/community/community-web-client/backup',
}
  ...
  beforeSetting () {
    ...
    return new Promise((resolve, reject) => {
      this.ssh.execCommand('mkdir -p ' + remotes.backupPath).then((result) => {
        if (!result.stderr) {
          console.log('beforeSetting success')
          resolve()
        } else {
          console.log('beforeSetting error' + JSON.stringify(result))
        }
      })
    })
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.ssh.execCommand('zip ' + remotes.backupPath + '/' + backupName + ' -r ./*', {cwd: remotes.scriptPath}).then((result) => {
        if (!result.stderr) {
          ...
          console.log('backup error ' + JSON.stringify(result))
        }
      })
    })
  }
      ...
      return new Promise((resolve, reject) => {
        _self.ssh.execCommand('rm -r ' + remotes.scriptPath).then((result) => {
          if (!result.stderr) {
            ...
            console.log('removeOldScript error ' + JSON.stringify(result))
          }
        })
      })
    }
