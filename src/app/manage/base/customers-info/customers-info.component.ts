...
export class CustomersInfoComponent extends TableBasicComponent
  implements OnInit {
  /* 面包屑导航 */
  public breadcrumbs = [
    {
  ...
  initFilterOption() {
    const dis = ZHANGZHOU_OPTIONS.filter(item => item.label === '福建省')[0]
      .children.filter(item => item.label === '漳州市')[0]
      .children.map(item => {
  ...
  onExp() {
    // this.customersInfoService.getCustomersReport(this.selectedItemCache.id).subscribe(
    //     res => {
        ...
        item.customerList.forEach((l: ListModel) => {
          ...
          if (l.id === target.id) {
            ...
            this.formCache = ModelConverter.customerResToFormModel(
              this.listResCache
                .find(
                  (customerResFirst: CustomerRes) =>
                    customerResFirst.id === item.id
                )
                .customerList.find(
                  (customerResSecond: CustomerRes) =>
                    customerResSecond.id === target.id
                ),
              item.name
            );
  ...
  dataToTableList(data: CustomerRes[]): ListModel[] {
    ...
    return data.map((o: CustomerRes) =>
      ModelConverter.customerResToListModel(o)
    );
  }
