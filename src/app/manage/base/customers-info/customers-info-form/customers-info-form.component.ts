...
export class CustomersInfoFormComponent implements OnInit {
  ...
  initForm() {
    if (this.cache) {
      ...
      this.customersInfoService.getCustomerById(this.cache.id).subscribe(
        res => {
          this.formModelCluster = ModelConverter.customerResToFormModel(
            res.data
          );
        },
        err => {}
      ...
      this.formModelSeparate.address = [
        '350000',
        '350600',
        '350603',
        '350603100'
  ...
  initVehicles() {
    const getRandomNameList = ({ districtCode, plateNumber }) => {
      ...
      // 若需要根据地区选择车辆,删除一下行代码
      districtCode = '';
      return this.customersInfoService
        .getCustomerVehicles(
          new PageReq(1, 20),
          districtCode,
          'Available',
    ...
    switch (this.type) {
      ...
      case 'edit':
        this.customersInfoService
          .updateCustomer(this.customerReq, this.cache.id)
          .subscribe(
            () => {
              this.notificationService.create({
                type: 'success',
  ...
  transformFormModelToRequest() {
    switch (this.selectedCategory) {
      case 'Separate':
        this.customerReq = ModelConverter.formModelToCustomerReq(
          this.formModelSeparate,
  ...
  onAddDuration(type: string): void {
    switch (this.selectedCategory) {
      case 'Separate' /* 普通点 | 子收运单位 */:
        const lengthS = this.formModelSeparate.duration[type]
          ? this.formModelSeparate.duration[type].length
          : 0;
        const newIdS = !lengthS
          ? 0
          : this.formModelSeparate.duration[type][lengthS - 1].idx + 1;
        this.formModelSeparate.duration[type].push(new DurationDetail(newIdS));
        break;
      case 'Cluster' /* 聚类点 */:
        const lengthC = this.formModelCluster.duration[type]
          ? this.formModelCluster.duration[type].length
          : 0;
        const newIdC = !lengthC
          ? 0
          : this.formModelCluster.duration[type][lengthC - 1].idx + 1;
  ...
  onRemoveDuration(durationType: string, idx: number) {
    switch (this.selectedCategory) {
      case 'Separate':
        ...
        const durationS = this.formModelSeparate.duration[durationType].filter(
          item => item.idx !== idx
        );
      ...
      case 'Cluster':
        ...
        const durationC = this.formModelCluster.duration[durationType].filter(
          item => item.idx !== idx
        );
  ...
  onAddChildCollection(): void {
    ...
    const newIdx = !length
      ? 0
      : this.formModelCluster.childCollections[length - 1].idx + 1;
  ...
  checkForm(): boolean {
    switch (this.selectedCategory) {
      case 'Separate':
        ...
        if (
          parseInt(this.formModelSeparate.hasKey, 10) !== 0 &&
          parseInt(this.formModelSeparate.hasKey, 10) !== 1
        ) {
          this.notificationService.create({
            type: 'error',
            title: '抱歉,请检查输入内容',
        ...
        if (
          (this.formModelSeparate.level === null ||
        ...
        if (this.formModelSeparate.duration.food.length) {
          let result = null;
          this.formModelSeparate.duration.food.forEach(
            (item: DurationDetail) => {
              // 判断是否有未填的必填数据
              const needComplete = !(
                !!item.dateType &&
              ...
              if (needComplete) {
                this.notificationService.create({
                  ...
                  title: '抱歉,请检查输入内容',
                  content:
                    '请检查收运时间段必填项的信息是否完善:【餐厨垃圾】时间类型/时间区间/重要等级'
                });
                result = false;
                return; // 停止循环
              } else {
        ...
        if (this.formModelSeparate.duration.oil.length) {
          let result = null;
          this.formModelSeparate.duration.oil.forEach(
            (item: DurationDetail) => {
              // 判断是否有未填的必填数据
              const needComplete = !(
                !!item.dateType &&
              ...
              if (needComplete) {
                this.notificationService.create({
                  ...
                  title: '抱歉,请检查输入内容',
                  content:
                    '请检查收运时间段必填项的信息是否完善:【油脂垃圾】时间类型/时间区间/重要等级'
                });
                result = false;
                return; // 停止循环
              } else {
      ...
      case 'Cluster':
        ...
        if (
          !this.formModelCluster.childCollections.length ||
          !this.formModelCluster.childCollections.filter(item => !!item.name)
            .length
        ) {
          this.notificationService.create({
            type: 'error',
        ...
        if (
          (this.formModelCluster.level === null ||
        ...
        if (this.formModelCluster.duration.food.length) {
          let result = null;
          this.formModelCluster.duration.food.forEach(
            (item: DurationDetail) => {
