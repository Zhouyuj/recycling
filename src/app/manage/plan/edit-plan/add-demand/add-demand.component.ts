...
export class AddDemandComponent extends TableBasicComponent implements OnInit {
  ...
  onChangeClusterTime(e) {
    if (e !== null) {
      this.selectedCluster.selectedPeriod = this.selectedCluster.collectionPeriods.find(
        (p: CollectionPeriod) => p.id === e
      );
    } else {
  ...
  onSubmit() {
    ...
    const selections: DemandListModel[] = this.demandListCache.filter(
      (item: DemandListModel) => item.checked
    );
    const req: DemandReq[] = selections.map((item: DemandListModel) =>
      ModelConverter.demandListModelToReq(item)
    );
  ...
  refreshSelectStatus(type: number): void {
    switch (type) {
      case 1:
        const allSelected = this.demandListCache.every(
          value => value.checked === true
        );
        const allUnSelected = this.demandListCache.every(
          value => !value.checked
        );
        ...
        break;
      case 2:
        const allSelectedSub = this.selectedCluster.taskList.every(
          value => value.checked === true
        );
        const allUnSelectedSub = this.selectedCluster.taskList.every(
          value => !value.checked
        );
    ...
    this.customersInfoService
      ...
      .subscribe(
        (res: Result<PageRes<CustomerRes[]>>) => {
          if (res.data) {
            this.demandListCache = this.customerResToTableRows(
              res.data.content
            );
  ...
  customerResToTableRows(res: CustomerRes[]): DemandListModel[] {
    return res.map((item: CustomerRes) =>
      ModelConverter.customerResToListModel(item)
    );
  }
  ...
  isValid(): boolean {
    let valid: boolean;
    const tempList = this.demandListCache.filter(
      (item: DemandListModel) => item.checked
    );
    ...
    tempList.forEach((item: DemandListModel) => {
      if (!item.collectionPeriodId) {
        ...
        // 存在子请求时
        item.taskList.forEach((child: SubDemandModel) => {
          if (child.checked && VerifyUtil.isEmpty(child.amountOfGarbage)) {
            // 子请求无输入收运量
            valid = false;
            return;
          } else {
        ...
        VerifyUtil.isEmpty(item.amountOfGarbage)
      ) {
        // 非聚类请求无输入收运量
        valid = false;
        return;
