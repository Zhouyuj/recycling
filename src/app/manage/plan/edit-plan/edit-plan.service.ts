...
import {
  Result,
  ResultError
} from '../../../shared/models/response/result.model';
...
export class EditPlanService extends RebirthHttp {
  ...
  @GET('/routes')
  getRouteList(
    @Query('planId') planId: number /* 编辑方案的id,必填 */,
    @Query('param') param?: { name?: string; plateNumber?: string }
  ...
  updateTasksPriorityOnRoute(
    @Path('id') id: number,
    @Body taskPriorityList: { id: number; priority: number }[]
  ): Observable<Result<any>> {
    return null;
  }
  ...
  ): Observable<Result<PageRes<DemandModel[]>>> {
    return null;
  }
  ...
  updateDemand(
    @Path('id') id: number,
    @Body demandDTO: { amountOfGarbage: number; collectionPeriodId: number }
  ): Observable<Result<any>> {
    return null;
  }
  ...
  editPlan(
    @Path('id') id: number,
    @Query('operate') operate: PlanOperationEnum
  ) {
    return null;
  }
