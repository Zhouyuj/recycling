...
export class PlanComponent extends TableBasicComponent implements OnInit {
  ...
  onNavigateToEditForRow(event: Event, item: PlanListModel) {
    this.listCache.forEach((plan: PlanListModel) => {
      plan.edit = false;
    });
    item.edit = true;
  }
  ...
  onNavigateToEdit() {
    this.planService
      .operatingPlan(this.selectedItem.id, PlanOperationEnum.EDIT)
      .subscribe(
  ...
  onExecute() {
    if (this.selectedItem.state) {
      this.planService
        .operatingPlan(this.selectedItem.id, PlanOperationEnum.EXECUTE)
        .subscribe(
          res => {
            console.log(res);
            this.getListByPage();
  ...
  onStop() {
    if (this.selectedItem.state) {
      this.planService
        .operatingPlan(this.selectedItem.id, PlanOperationEnum.STOP)
        .subscribe(
          res => {
            this.getListByPage();
            this.notificationService.create({
  ...
  updateParams() {
    ...
    for (const k of Object.keys(this.params)) {
      ...
      if (k === 'status') {
        // 需求:方案管理页面只显示 【未执行】和【执行中】,而不显示【已停止】和【已完成】
        if (this.params[k] === null) {
          this.params[k] =
            PlanStateEnum.Executing + ',' + PlanStateEnum.UnExecuted;
          paramsTemp[k] = this.params[k];
        }
      }
    }
