...
export class AddPlanComponent extends TableBasicComponent implements OnInit {
  @Input() title: string;
  ...
  getListByPage() {
    ...
    this.planService
      .getPlanList(this.pageReq, paramsTemp)
      .subscribe((res: Result<PageRes<PlanRes[]>>) => {
        if (res.data.content) {
          this.resCache = res.data.content;
          this.listCache = res.data.content.map(item =>
            ModelConverter.planResToPlanListModel(item)
          );
  ...
  onSubmitExistPlan() {
    ...
    this.planService
      .addPlan(this.formDataExist, `${this.selectedItem.id}`)
      .subscribe(
        res => {
  ...
  onCheckedChange(e?: boolean, item?: PlanListModel) {
    ...
    this.selectedItem = this.resCache.filter(
      (o: PlanRes) => o.id === item.id
    )[0];
