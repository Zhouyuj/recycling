...
export class Marker {
    ...
    public zIndex: number;          // 点标记的叠加顺序。地图上存在多个点标记叠加时，通过该属性使级别较高的点标记在上层显示default 100
...
export class MarkerIcon {
    ...
    constructor(size: number[], image: string, imageOffset: number[] = [ 0, 0 ], imageSize?: number[]) {
        this.size = size;
        this.image = image;
